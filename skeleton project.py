...
def UpdateTargets(Targets, TrainingGame, MaxTarget):
    for Count in range (0, len(Targets) - 1): #if len of the list is 5, the range will be range(4), the highest it will reach is 3, as range isn't inclusive
        Targets[Count] = Targets[Count + 1] #the highest count is 3, so the number addressed in count+1 will be 4, which is the highest list index
    Targets.pop()
    if TrainingGame:
...
def DisplayScore(Score):
    ...
    if Score > 10:
        print("The score is more than 10!")
    print()
    print()
...
def ConvertToRPN(UserInput):
    Position = 0
    Precedence = {"+": 2, "-": 2, "*": 4, "/": 4} #what about brackets
    Operators = []
